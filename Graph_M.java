...
public class Graph_M {
    ...
    public int dijkstra(String src, String des, boolean nan) {
        ...
        Heap_M<DijkstraPair> heap = new Heap_M<>();
    ...
    public static void Create_Metro_Map(Graph_M g) {
        ...
        g.addVertex("Esplanade~BG");
        g.addVertex("Mahakaran~G");
        g.addVertex("Howrah~G");
        g.addVertex("Howrah Maidan~G");
        g.addVertex("Central~B");
        g.addVertex("M G Road~B");
        g.addVertex("Shovabazar Sutanuti~B");
        g.addVertex("Shyambazar~B");
        g.addVertex("Belgachhia~B");
        g.addVertex("Dumdum~B");
        g.addVertex("Noapara~BY");
        g.addVertex("Dakshineswar~B");
        g.addVertex("Saltlake Sector V~G");
        g.addVertex("Karunamoyee~G");
        g.addVertex("City Center~G");
        g.addVertex("Saltlake Stadium~G");
        g.addVertex("Sealdah~G");
        g.addVertex("Satyajit Ray~O");
        g.addVertex("Kavi Sukanta~O");
        g.addVertex("Hemanta Mukhopadhyay~O");
        g.addVertex("Joka~P");
        g.addVertex("Thakurpukur~P");
        g.addVertex("Behala Chowrasta~P");
        g.addVertex("Taratala~P");
        g.addVertex("Majherhat~P");
        


        g.addEdge("Dakshineswar~B", "Noapara~BY", 3);
        g.addEdge("Noapara~BY", "Dumdum~B", 2);
        g.addEdge("Dumdum~B", "Belgachhia~B", 1);
        g.addEdge("Belgachhia~B", "Shyambazar~B", 1);
        g.addEdge("Shyambazar~B", "Shovabazar Sutanuti~B", 1);
        g.addEdge("Shovabazar Sutanuti~B", "M G Road~B", 1);
        g.addEdge("M G Road~B", "Central~B", 1);
        g.addEdge("Central~B", "Esplanade~BG", 1);
        g.addEdge("Esplanade~BG", "Park Street~B", 1);
        g.addEdge("Park Street~B", "Maidan~B", 0);
        g.addEdge("Maidan~B", "Rabindra Sadan~B", 1);
        g.addEdge("Rabindra Sadan~B", "Kalighat~B", 3);
        g.addEdge("Kalighat~B", "Rabindra Sarobar~B", 1);
        g.addEdge("Rabindra Sarobar~B", "Mahanayak Uttam Kumar~B", 1);
        g.addEdge("Mahanayak Uttam Kumar~B", "Masterda Surya Sen~B", 3);
        g.addEdge("Masterda Surya Sen~B", "Kavi Nazrul~B", 2);
        g.addEdge("Kavi Nazrul~B", "Kavi Subhash~BO", 1);
        g.addEdge("Kavi Subhash~BO", "Satyajit Roy~O", 1);
        g.addEdge("Satyajit Roy~O", "Kavi Sukanta~O", 4);
        g.addEdge("Kavi Sukanta~O", "Hemanta Mukhopadhyay~O", 2);
        g.addEdge("Esplanade~BG", "Mahakaran~G", 1);
        g.addEdge("Mahakaran~G", "Howrah~G", 3);
        g.addEdge("Howrah~G", "Howrah Maidan~G", 0);
        g.addEdge("Saltlake Sector V~G", "Karunamoyee~G", 8);
        g.addEdge("Karunamoyee~G", "City Center~G", 1);
        g.addEdge("City Center~G", "Saltlake Stadium~G", 1);
        g.addEdge("Saltlake Stadium~G", "Sealdah~G", 3);
        g.addEdge("Joka~P", "Thakurpukur~P", 1);
        g.addEdge("Thakurpukur~P", "Behala Chowrasta~P", 2);
        g.addEdge("Behala Chowrasta~P", "Taratala~P", 2);
        g.addEdge("Taratala~P", "Majherhat~P", 0);
    }
    ...
    public int calculateFare(String src, String dest) {
        // Get the distance between source and destination stations
        int distance = dijkstra(src, dest, false);
    
        // Define fare calculation logic based on distance
        int fare;
    
        if (distance <= 2) {
            fare = 5; // Fare for distances up to 2km
        } else if (distance <= 5) {
            fare = 10; // Fare for distances between 2km and 5km
        } else if (distance <= 10) {
            fare = 15; // Fare for distances between 5km and 10km
        } else if (distance <= 15) {
            fare = 20; // Fare for distances between 10km and 15km
        } else if (distance <= 25) {
            fare = 25; // Fare for distances between 15km and 25km
        } else {
            fare = 30; // Fare for distances above 25km
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (true) {
            ...
            System.out.println("7. SHOW THE FARE FROM 'SOURCE' STATION TO 'DESTINATION' STATION");
            ...
            switch (choice) {
                ...
                case 7:
                    ...
                    System.out.println("THE FARE TO TRAVEL FROM " + src + " to " + dest + " is " + g.calculateFare(src, dest));
                    System.out.println();
                    System.out.println();
                    break;
